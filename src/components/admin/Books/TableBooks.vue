...
const fetchBooks = async (id: any) => {
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/products/?page=${id}&limit=10`
    );
...
const handleJumpEdit: (id: string) => void = (id) => {
  router.push({ path: `/admin/books/edit/${id}` });
};
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-6 flex item-scenter gap-3 py-4 text-center text-lg">
              ...
              <button
                type="button"
                @click="handleJumpEdit(item.id)"
                class="btn w-12 btn-square btn-info"
              >
                <svg
                  viewBox="0 0 24 24"
                  fill="none"
